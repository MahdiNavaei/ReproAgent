...
def test_pytest_fixture_asserts_regression(
    agentcase_regression: object,
) -> None:
    assert callable(agentcase_regression)
    baseline = _baseline()
    agentcase_regression(baseline, baseline)  # type: ignore[operator]
